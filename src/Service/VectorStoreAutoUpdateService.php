 ...
 *
 * run() is invoked by the cron job and by the contao:openai-vector-sync command.
 * The backend manual trigger calls dispatchRun() (non-blocking CLI dispatch) only,
 * never run() inline — the crawl + LLM call can take minutes (constraint C4).
 */
class VectorStoreAutoUpdateService
{
        ...
        try {
            ...
            if (\count($rows) === 0) {
                throw new \RuntimeException(
                    'tl_search is empty. Run a search re-index in the Contao backend (System → Maintenance) before enabling auto-update.'
                );
            }
            ...
            $this->persistResult($configId, 'success', $newFileId, [
                'pages'      => \count($rows),
            ...
            $this->persistResult($configId, 'error', $oldFileId, [
                'duration' => time() - $start,
            ], $e->getMessage());
        }
    }
    ...
    private function readSearchIndex(int $configId): array
    {
        ...
        if ($rootId <= 0) {
            if (\count($roots) === 1) {
                ...
                throw new \RuntimeException(
                    'Multiple site roots detected. Select a site root in OpenAI Configuration → Automatic vector store sync → Site root.'
                );
            } else {
                return [];
            }
     ...
     */
    private function generateDocument(string $apiKey, string $model, string $pageContent, ?string $promptTemplate): array
    {
        ...
        $response = $this->http->request('POST', self::OPENAI_BASE . '/chat/completions', [
            'headers' => [
    ...
    private function deleteOldFile(string $apiKey, string $vectorStoreId, string $oldFileId): void
    {
        ...
        try {
            $this->http->request('DELETE', self::OPENAI_BASE . "/vector_stores/{$vectorStoreId}/files/{$oldFileId}", [
                'headers' => [
                    'Authorization' => 'Bearer ' . $apiKey,
        ...
        try {
            $this->http->request('DELETE', self::OPENAI_BASE . "/files/{$oldFileId}", [
                'headers' => [
                    'Authorization' => 'Bearer ' . $apiKey,
                ],
                'timeout' => 30,
    ...
    {
        // Symfony HttpClient multipart needs a real stream — write to a temp file.
        // Use a cryptographically random name and exclusive create ('xb') so a local
        ...
        try {
            ...
            $response = $this->http->request('POST', self::OPENAI_BASE . '/files', [
                'headers' => [
                    'Authorization' => 'Bearer ' . $apiKey,
                ],
                'body'    => [
    ...
    {
        $this->http->request('POST', self::OPENAI_BASE . "/vector_stores/{$vectorStoreId}/files", [
            'headers' => [
                'Authorization' => 'Bearer ' . $apiKey,
                'Content-Type'  => 'application/json',
                'OpenAI-Beta'   => 'assistants=v2',
