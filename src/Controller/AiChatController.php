...
class AiChatController extends AbstractController
{
    ...
    public function send(Request $request): JsonResponse
    {
        ...
        if (!$request->isXmlHttpRequest()) {
            return new JsonResponse(['error' => $this->getErrorMessage('invalid_request', $language)], 400);
        }
        ...
        if (!$submittedToken) {
            return new JsonResponse(['error' => $this->getErrorMessage('csrf_token_missing', $language)], 400);
        }

        $token = new CsrfToken($this->csrfTokenName, $submittedToken);
        if (!$this->csrfTokenManager->isTokenValid($token)) {
            return new JsonResponse(['error' => $this->getErrorMessage('invalid_csrf_token', $language)], 403);
        }

        // Get and validate message
        $message = trim($request->request->get('message', ''));
        if (empty($message)) {
            return new JsonResponse(['error' => $this->getErrorMessage('empty_message', $language)], 400);
        }
        ...
        if (($currentTime - $lastRequest) < 2) {
            return new JsonResponse(['error' => $this->getErrorMessage('please_wait', $language)], 429);
        }
        ...
        try {
            ...
            return new JsonResponse(['error' => $this->getErrorMessage('service_unavailable', $language)], 500);
        }
    }
    ...
    public function getToken(Request $request): JsonResponse
    {
        ...
        if (($currentTime - $lastTokenRequest) < 10) {
            return new JsonResponse(['error' => $this->getErrorMessage('token_requests_too_frequent', $language)], 429);
        }
