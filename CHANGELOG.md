...
- The chatbot question field now keeps its keyboard focus ring fully visible.

## [2.2.2] - 2026-09-07

### Changed

- Updates the premium links in the backend.
- Renames the premium product to Contao OpenAI Assistant Premium.
...
> Run `contao:migrate` immediately after deploying the code. The first premium
> synchronisation rebuilds the knowledge base once — a longer crawl and, in
> "KI-optimiert" mode, a one-off token cost — then later runs are incremental again.
>
> The [detailed v2.2.0 release notes](https://github.com/juhe-it-solutions/contao-openai-assistant/blob/v2.2.0/docs/releases/2.2.0-detailed.md) preserve the
> complete technical record of all 55 changes.

### Added

- **Synchronisations are faster when the website is unchanged (premium).** Automatic search-index refreshes can skip the full-site crawl when nothing changed, with a forced refresh at least every six hours; no-change runs now say clearly that nothing was transferred. See [what gets indexed](https://github.com/juhe-it-solutions/contao-openai-assistant/blob/v2.2.0/docs/features/indexing.md).
- **"KI-optimiert" reuses unchanged AI-polished documents (premium).** The cache avoids repeatedly rewriting and uploading identical pages while still invalidating when page text, the model, the prompt or relevant boilerplate changes.
- **Chatbot answers can include verified page, document and external links (premium).** Link collection, filtering and an optional directory are configurable, and each chat module can choose where links open. See [page links](https://github.com/juhe-it-solutions/contao-openai-assistant/blob/v2.2.0/docs/features/page-links.md) and [link targets](https://github.com/juhe-it-solutions/contao-openai-assistant/blob/v2.2.0/docs/features/link-target.md).
- **The vector-store file overview connects OpenAI files to their source pages (premium).** It includes indexed-text inspection, searchable file IDs and richer downloadable run manifests.
- **The synchronisation dashboard reports useful progress and outcomes (premium).** It shows start time, duration, measurable upload/AI progress, honest crawl activity and clearer no-change or warning messages.

### Changed

- **The crawler now reaches pages beyond three link levels, and news, FAQ and event entries have a separate subscription allowance (premium).** Previously unreachable content can enter the knowledge base; exceeding the entry allowance reports a partial run but does not remove or block content. See [what gets indexed](https://github.com/juhe-it-solutions/contao-openai-assistant/blob/v2.2.0/docs/features/indexing.md).
- **Synchronisation settings are easier to operate.** Link and schedule options are grouped, expensive schedules are flagged using real run duration, and reader-page documents use the page's own title and URL.

### Fixed

- **The 2.2 migration keeps the old bulk knowledge document until all replacement page files work (premium).** OpenAI ingestion and deletion are confirmed, failed candidates are rolled back, and unresolved remote operations remain visible and retryable.
- **Deleted, unpublished, deselected and protected pages are reliably removed (premium).** Protection is resolved from the page tree, including inheritance, without relying on a stale search-index entry.
- **Manual uploads and vector-store attachments no longer report success before every step succeeds.** Failed attachment or database writes trigger checked rollback instead of leaving unreachable files behind.
- **Daily chat limits are atomic and recover correctly.** Concurrent requests cannot overshoot the cap; provably unbilled failures release their reservation, limit increases apply immediately, and timeout/retry handling avoids duplicate answers. See [rate limiting and cost protection](https://github.com/juhe-it-solutions/contao-openai-assistant/blob/v2.2.0/docs/security/rate-limiting.md).
- **The frontend chatbot is more robust and accessible.** Editor-managed HTML renders safely, mobile keyboard and safe-area handling is reliable, reduced-motion preferences are honoured, and the disclaimer is a true modal dialog with structured keyboard navigation. See [HTML in chat texts](https://github.com/juhe-it-solutions/contao-openai-assistant/blob/v2.2.0/docs/features/chat-text-html.md) and the [disclaimer feature](https://github.com/juhe-it-solutions/contao-openai-assistant/blob/v2.2.0/docs/features/disclaimer.md).
- **Configuration, API-key checks and scheduled synchronisation report failures accurately.** Duplicate callbacks, misleading offline validation, stale cron warnings, missing-migration database errors, subdirectory endpoints and several recovery/logging edge cases are corrected.

### Security

- **Protected member content is excluded from the public chatbot's knowledge base (premium).** Newly protected pages and inherited protection are detected before synchronisation and removed from stored links and files.
- **The public chat endpoint has stronger cost and session protections.** Chat-history reads require the security token, messages are limited to 4,000 characters, and the documentation now makes clear that hiding the module does not make the endpoint private. See [rate limiting and cost protection](https://github.com/juhe-it-solutions/contao-openai-assistant/blob/v2.2.0/docs/security/rate-limiting.md).
