...
- **The dark chat input stays readable while focused when Contao's Basic forms stylesheet is enabled.** Its generic `textarea:focus` rule could replace the configured dark background with a light one while leaving the light text unchanged. The widget now reapplies both theme colours in a scoped focus rule on Contao 5.3, 5.7 and 6.0.
...
- **The daily message limit is no longer used up by requests that provably could not be billed.** Failed conversation creation and Responses requests rejected with a 4xx or 503 cannot invoke a model completion, so they return their reserved slot. Ambiguous read timeouts and other server failures still keep it, because the model may already have run.
...
- **A question that takes too long is no longer answered twice.** The chat widget gives up after two minutes and invites the visitor to try again, but the server was allowed three - and closing the connection in the browser does not stop the work already running on the server. The first question could therefore still be answered, be billed, and be added to the conversation after the visitor had been told it failed, so the retry arrived into a conversation that had silently moved on. One server deadline now covers the complete turn - initial conversation creation, the Responses call, automatic repeats and stale-conversation recovery - and remains below the widget's two-minute deadline instead of restarting for each phase.
