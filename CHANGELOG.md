...
- **Leftover theme site roots no longer block the automatic synchronisation (premium).** When no pages are selected explicitly, the synchronisation covers the whole website - but only if exactly one site root exists. Site roots that are not live were counted too, so an installation with a single live website could fail with "more than one site root found" - typically because of an unpublished site root left behind by a theme import. Only roots that are actually live count now: published, and within their start/stop dates if those are set (the same rule Contao itself applies to a page).
