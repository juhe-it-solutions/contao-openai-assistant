...
- **Leftover theme site roots no longer block the automatic synchronisation (premium).** When no pages are selected explicitly, the synchronisation covers the whole website - but only if exactly one site root exists. Unpublished site roots left behind by a theme import were counted too, so an installation with a single live website could fail with "more than one site root found". Only published site roots are considered now.
