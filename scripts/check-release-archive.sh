...
# Verify the exact runtime file set Composer and GitHub export for a release tag.
...
FORBIDDEN_PATTERN='(^|/)(\.env([^/]*|$)|XXX_DOCS_INTERN|vendor|node_modules|dist|build|coverage|logs?|secrets?)(/|$)|(^|/)composer\.lock$|\.(bak|backup|crt|db|key|log|orig|p12|pem|rej|sqlite|temp|tmp)$'
if grep -Eiq "$FORBIDDEN_PATTERN" "$ARCHIVE_DIR/tracked-entries.txt"; then
    echo "Forbidden files tracked by the release tag:" >&2
    grep -Ei "$FORBIDDEN_PATTERN" "$ARCHIVE_DIR/tracked-entries.txt" >&2
...
if awk '$1 !~ /^[-d]/ { print; found = 1 } END { exit !found }' "$ARCHIVE_DIR/entry-details.txt" > "$ARCHIVE_DIR/non-regular-entries.txt"; then
    echo "Unsupported archive entries:" >&2
...
ARCHIVE_FORBIDDEN='(^|/)(\.env([^/]*|$)|\.git(hub|ignore|attributes)?|XXX_DOCS_INTERN|docs|scripts|tests|vendor|node_modules|dist|build|coverage|cache|logs?|secrets?)(/|$)|(^|/)composer\.lock$|\.(bak|backup|crt|db|key|log|orig|p12|pem|rej|sqlite|temp|tmp)$'
if grep -Eiq "$ARCHIVE_FORBIDDEN" "$ARCHIVE_DIR/entries.txt"; then
    echo "Forbidden archive entries:" >&2
    grep -Ei "$ARCHIVE_FORBIDDEN" "$ARCHIVE_DIR/entries.txt" >&2
    fail "development, internal, or secret-bearing file names were exported"
fi

SENSITIVE_PATTERN='(-----BEGIN (RSA |EC |OPENSSH |DSA )?PRIVATE KEY-----|sk-(proj-)?[A-Za-z0-9_-]{32,}|github_pat_[A-Za-z0-9_]{20,}|gh[pousr]_[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16}|XXX_DOCS_INTERN|/home/julle|/mnt/c/Users|AppData/Local/Temp)'
set +e
grep -RIlE --exclude=package.tar --exclude=entries.txt --exclude=entry-details.txt \
    ...
    "$SENSITIVE_PATTERN" "$ARCHIVE_DIR" > "$ARCHIVE_DIR/sensitive-files.txt"
SENSITIVE_STATUS=$?
set -e

if [ "$SENSITIVE_STATUS" -eq 0 ]; then
    echo "Exported files matching the sensitive-content scan:" >&2
    cat "$ARCHIVE_DIR/sensitive-files.txt" >&2
    fail "potential credential or local machine path found in the exported package"
fi

if [ "$SENSITIVE_STATUS" -ne 1 ]; then
    fail "the exported-package sensitive-content scan could not be completed"
fi
